...
async function sendChat() {
    ...
    <main style={{ background: "#000", color: "#fff" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .nav-a { color: #666; font-size: 13px; letter-spacing: 0.01em; transition: color 0.2s; }
      ...
      {/* Nav */}
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 300, height: 56, display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0 48px", background: "rgba(0,0,0,0.85)", backdropFilter: "blur(16px)", borderBottom: "1px solid #1a1a1a" }}>
  ...
  <a href="/chat" className="nav-a">Ask AI</a>
        ...
        <a href="/hire" style={{ background: "#fff", color: "#000", padding: "8px 20px", fontSize: 12, fontWeight: 700, letterSpacing: "0.05em", textTransform: "uppercase" }}>Hire an agent →</a>
      </nav>
      ...
      <section style={{ position: "relative", height: "100vh", background: "#000", overflow: "hidden" }}>
        ...
        <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1 }}>
          <h1 className="serif" style={{ fontSize: "clamp(72px,14vw,180px)", fontWeight: 900, letterSpacing: "-0.03em", lineHeight: 0.9, color: "#fff", textAlign: "center", animation: "fadeIn 1.5s ease" }}>
            Odyssey
          </h1>
        </div>
        <div style={{ position: "absolute", bottom: 40, left: 48, zIndex: 2 }}>
          <p className="mono" style={{ fontSize: 10, color: "#555", letterSpacing: "0.2em", textTransform: "uppercase" }}>// AI Agent Marketplace on Base</p>
        </div>
        <div style={{ position: "absolute", bottom: 40, right: 48, zIndex: 2, display: "flex", gap: 12 }}>
          <a href="/hire" style={{ background: "#fff", color: "#000", padding: "10px 24px", fontSize: 12, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase" }}>Hire an agent</a>
          <a href="/how" style={{ border: "1px solid #333", color: "#888", padding: "10px 24px", fontSize: 12, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase" }}>How it works</a>
        </div>
      </section>
      ...
      <section style={{ background: "#f4f4f2", color: "#000", padding: "100px 48px" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 80, alignItems: "start" }}>
            <h2 className="serif" style={{ fontSize: "clamp(40px,5.5vw,76px)", fontWeight: 700, lineHeight: 1.05, letterSpacing: "-0.02em", color: "#000" }}>
              The AI Agent<br />Marketplace.
            </h2>
            <div style={{ paddingTop: 8 }}>
              <p style={{ fontSize: 18, color: "#666", lineHeight: 1.85, marginBottom: 40 }}>
      ...
      {/* Split — dark lines left, purple right */}
      <section style={{ display: "grid", gridTemplateColumns: "55% 45%", minHeight: "55vh" }}>
        ...
        <div style={{ background: "#5555ff", color: "#fff", padding: "80px 64px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
          <p className="mono" style={{ fontSize: 10, color: "rgba(255,255,255,0.4)", letterSpacing: "0.2em", textTransform: "uppercase", marginBottom: 32 }}>// the execution layer</p>
          <h2 className="serif" style={{ fontSize: "clamp(32px,4vw,56px)", fontWeight: 700, lineHeight: 1.15, letterSpacing: "-0.02em" }}>
            Agents that pay for themselves.
          </h2>
          <p style={{ fontSize: 15, color: "rgba(255,255,255,0.65)", lineHeight: 1.8, marginTop: 24 }}>Every tool call is a real USDC transaction on Base. The agent earns, spends, and accounts — autonomously.</p>
        </div>
      </section>
      ...
      <section style={{ background: "#000", borderTop: "1px solid #1a1a1a", borderBottom: "1px solid #1a1a1a" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)" }}>
          {[
            ...
            <div key={s.label} style={{ padding: "52px 48px", borderRight: i < 2 ? "1px solid #1a1a1a" : "none" }}>
              <p className="serif" style={{ fontSize: "clamp(32px,4vw,56px)", fontWeight: 700, color: "#fff", marginBottom: 12, letterSpacing: "-0.02em" }}>{s.value}</p>
      ...
      {/* Agents — editorial list style */}
      <section style={{ background: "#000", padding: "100px 48px" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          <p className="mono" style={{ fontSize: 10, color: "#444", textTransform: "uppercase", letterSpacing: "0.2em", marginBottom: 60 }}>// agents</p>
          <h2 className="serif" style={{ fontSize: "clamp(52px,8vw,108px)", fontWeight: 900, letterSpacing: "-0.03em", lineHeight: 0.92, color: "#fff", marginBottom: 80 }}>
            Three agents.<br />Three jobs.
          </h2>
          {[
            { n: "01", name: "Competitor Intelligence", price: "$0.10", desc: "Describe your product. The agent reads your competitors' websites and tells you exactly where you win and where you lose.", tools: ["Brave Search", "Exa", "Claude"], time: "~2 min", profit: "$0.052 profit" },
            { n: "02", name: "Lead Generation", price: "$0.20", desc: "Define your ideal customer. The agent finds 10 matching companies, gathers their details, and writes personalized outreach for each.", tools: ["Exa", "Apollo", "Claude"], time: "~3 min", profit: "$0.177 profit" },
            { n: "03", name: "Market Research", price: "$0.05", desc: "Name a topic. The agent scans the web for what is happening right now and returns a brief you can act on.", tools: ["Brave Search", "Claude"], time: "~90 sec", profit: "$0.009 profit" },
          ].map(s => (
            <div key={s.n} className="agent-row" style={{ padding: "40px 0", display: "grid", gridTemplateColumns: "80px 1fr 1fr auto", gap: 48, alignItems: "center" }}>
              <p className="mono" style={{ fontSize: 12, color: "#333", letterSpacing: "0.1em" }}>{s.n}</p>
              <h3 className="serif" style={{ fontSize: "clamp(22px,2.5vw,32px)", fontWeight: 700, color: "#fff", letterSpacing: "-0.02em" }}>{s.name}</h3>
              <p style={{ fontSize: 14, color: "#555", lineHeight: 1.75 }}>{s.desc}</p>
              <div style={{ textAlign: "right" }}>
                <p className="serif" style={{ fontSize: 28, fontWeight: 700, color: "#fff", marginBottom: 8 }}>{s.price}</p>
                <p className="mono" style={{ fontSize: 10, color: "#10b981", marginBottom: 12, letterSpacing: "0.08em" }}>{s.profit}</p>
                <a href="/hire" style={{ display: "inline-block", background: "#fff", color: "#000", padding: "8px 20px", fontSize: 11, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase" }}>Hire →</a>
              </div>
            </div>
          ))}
        </div>
      ...
      <section style={{ background: "#f4f4f2", color: "#000", padding: "100px 48px" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 80, alignItems: "start" }}>
            <div>
              <h2 className="serif" style={{ fontSize: "clamp(36px,5vw,68px)", fontWeight: 700, lineHeight: 1.05, letterSpacing: "-0.02em", marginBottom: 28 }}>
      ...
      {/* Predictions CTA — split */}
      <section style={{ display: "grid", gridTemplateColumns: "45% 55%", minHeight: "50vh" }}>
        <div style={{ background: "#10b981", color: "#000", padding: "80px 64px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
          <p className="mono" style={{ fontSize: 10, color: "rgba(0,0,0,0.4)", letterSpacing: "0.2em", textTransform: "uppercase", marginBottom: 32 }}>// prediction markets</p>
          <h2 className="serif" style={{ fontSize: "clamp(32px,4vw,56px)", fontWeight: 700, lineHeight: 1.1, letterSpacing: "-0.02em", marginBottom: 32 }}>
            Bet on outcomes.<br />Win real USDC.
          </h2>
          <p style={{ fontSize: 15, color: "rgba(0,0,0,0.6)", lineHeight: 1.8, marginBottom: 40, maxWidth: 380 }}>Every market is auto-generated from real agent research. Bet YES or NO. Winners take the pot.</p>
      ...
      {/* Footer */}
      <footer style={{ background: "#000", borderTop: "1px solid #1a1a1a", padding: "48px 48px", display: "grid", gridTemplateColumns: "1fr auto auto", gap: 80, alignItems: "start" }}>
        <div>
        ...
        <div>
          ...
          {[{ href: "/hire", label: "Agents" }, { href: "/predictions", label: "Predictions" }, { href: "/ledger", label: "Ledger" }, { href: "/portfolio", label: "Portfolio" }, { href: "/about", label: "Docs" }].map(l => (
            <a key={l.href} href={l.href} style={{ display: "block", fontSize: 13, color: "#555", marginBottom: 10, transition: "color 0.2s" }}>{l.label}</a>
          ))}
        </div>
        <div>
...
<div style={{ position: "fixed", bottom: 32, right: 32, zIndex: 999 }}>
  {chatOpen && (
    <div style={{ position: "absolute", bottom: 64, right: 0, width: 360, background: "#fff", border: "1px solid #e5e5e5", boxShadow: "0 8px 40px rgba(0,0,0,0.15)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      {/* Chat header */}
      ...
      <div style={{ height: 280, overflowY: "auto", padding: "16px", display: "flex", flexDirection: "column", gap: 12 }}>
        {chatMessages.map((m, i) => (
          <div key={i} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start" }}>
            <div style={{
              background: m.role === "user" ? "#000" : "#f4f4f2",
      ...
      <div style={{ padding: "12px", borderTop: "1px solid #e5e5e5", display: "flex", gap: 8 }}>
        ...
        />
        <button onClick={sendChat} disabled={chatLoading || !chatInput.trim()} style={{ background: "#000", color: "#fff", border: "none", padding: "8px 16px", fontSize: 12, fontWeight: 700, cursor: chatLoading ? "not-allowed" : "pointer", opacity: chatLoading ? 0.5 : 1, fontFamily: "inherit", letterSpacing: "0.05em" }}>
          →
        </button>
      </div>
    </div>
  ...
  {/* Bubble button */}
  <button onClick={() => setChatOpen(o => !o)} style={{ width: 52, height: 52, borderRadius: "50%", background: "#fff", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 20px rgba(0,0,0,0.3)", fontSize: 22, transition: "transform 0.2s" }}>
    {chatOpen ? "✕" : "💬"}
  </button>
</div>
    </main>
